...
@section('title', 'Edit Koleksi Global - Literasia')

@section('content')
<div x-data="libraryCreate()" class="max-w-4xl mx-auto space-y-6">
    <!-- Header -->
    <div class="flex items-center gap-4">
        <a href="{{ route('dinas.library') }}" class="p-2.5 rounded-xl bg-white border border-slate-200 text-slate-400 hover:text-[#d90d8b] hover:border-[#d90d8b] transition-all">
            <i class="material-icons">arrow_back</i>
        </a>
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Edit Koleksi Global</h1>
            <p class="text-slate-500">Ubah informasi atau fail materi digital perpustakaan untuk semua sekolah.</p>
        </div>
    </div>

    <!-- Form Card -->
    <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
        <form id="libraryForm" action="{{ route('dinas.library.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
            @csrf
            @method('PUT')
            ...
            async submitForm() {
                ...
                try {
                    ...
                    Swal.fire({
                        ...
                        window.location.href = '{{ route("dinas.library") }}';
                    });
                    ...
                    Swal.fire({
                        ...
                        title: 'Gagal',
                        text: msg,
                        footer: '<p class="text-xs text-slate-400 text-center">Cek Console (F12) untuk detail teknis atau status CORS storage.</p>'
                    });
                }
            }
        }
