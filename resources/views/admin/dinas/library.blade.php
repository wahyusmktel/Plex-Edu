        ...
        @forelse($items as $item)
            <div class="group bg-white rounded-3xl border border-slate-100 overflow-hidden hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-500 flex flex-col">
                <div class="aspect-[3/4] bg-slate-100 relative overflow-hidden">
                    @if($item->cover_image)
                        <img src="{{ Storage::temporaryUrl($item->cover_image, now()->addHours(1)) }}" alt="{{ $item->title }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                    @else
                    ...
                    <div class="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 z-20">
                        ...
                        <button type="button" 
                            @click="$store.reader.openModal('{{ addslashes($item->title) }}', '{{ Storage::temporaryUrl($item->file_path, now()->addHours(1)) }}')" 
                        ...
                        <button type="button" 
                            @click="$store.videoPlayer.openModal('{{ addslashes($item->title) }}', '{{ Storage::temporaryUrl($item->file_path, now()->addHours(1)) }}')"
                        ...
                        <button @click="playAudio('{{ Storage::temporaryUrl($item->file_path, now()->addHours(1)) }}', '{{ addslashes($item->title) }}')" class="w-12 h-12 rounded-full bg-white text-slate-800 flex items-center justify-center hover:bg-orange-500 hover:text-white transition-all shadow-xl cursor-pointer">
